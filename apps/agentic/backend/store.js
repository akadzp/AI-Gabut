...
const SCOPES = ["users","auth-sessions","chat-sessions","connections","workspaces","tasks","executions","activities"];
export function createAgenticStore({storage=createStorage()}={}) { const state=createStateService({storage,namespace:"agentic",scopes:SCOPES});
 ...
 async function putOwned(scope,u,id,value,options={overwrite:true}) { return state.put(scope,ownerKey(u,id),{...value,id,ownerId:u},options); }
 ...
 return Object.freeze({state,createId,putOwned,getOwned,listOwned,removeOwned,
  ...
  putTask:(u,t)=>putOwned("tasks",u,t.id,t),getTask:(u,id)=>getOwned("tasks",u,id),listTasks:u=>listOwned("tasks",u),
  putExecution:(u,e)=>putOwned("executions",u,e.id,e),getExecution:(u,id)=>getOwned("executions",u,id),listExecutions:u=>listOwned("executions",u),
  putActivity:(u,a)=>putOwned("activities",u,a.id,a),listActivities:u=>listOwned("activities",u)
 }); }
